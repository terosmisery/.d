...
Title.TextSize = 18
Title.Text = "INC FE v" .. currentVersion
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- INC Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
CMDs = {}
CMDs[#CMDs + 1] = {NAME = 'discord / support / help', DESC = 'Invite to the INC discord server.'}
...
-- ================================================================
-- INC CUSTOM FRONT-END
-- The original command engine above remains intact. This front-end
-- calls the original execCmd()/findCmd() implementations so the
-- catalogue is functional instead of being a display-only list.
-- ================================================================
...
do
    ...
    pcall(function() Settings.Visible = false end)
    pcall(function() KeybindsFrame.Visible = false end)
    pcall(function() PositionsFrame.Visible = false end)
    pcall(function() AliasesFrame.Visible = false end)
    pcall(function() PluginsFrame.Visible = false end)
    ...
    local CONFIG = {
        ...
        Client = Color3.fromRGB(130, 190, 255),
        Server = Color3.fromRGB(130, 255, 165),
    }
    ...
    local sectionNames = {
        [0]="General", [1]="Player Physics", [2]="Waypoints", [3]="Teleportation",
        [4]="Chat & Logs", [5]="Visuals", [6]="Camera", [7]="Building & World",
        [8]="Lighting", [9]="Player Information & Fun", [10]="Character",
        [11]="Animations", [12]="Input & Mouse", [13]="Tools", [14]="Aliases", [15]="Plugins"
    }

    local function cleanINC(text)
    ...
    -- CMDs is the original catalogue; cmds is the original executable registry.
    -- Blank catalogue items delimit its original sections.
    if type(CMDs) == "table" then
        for _, item in ipairs(CMDs) do
            local rawName = tostring(item.NAME or "")
            local rawDesc = tostring(item.DESC or "")
            if rawName == "" and rawDesc == "" then
                sectionIndex = sectionIndex + 1
            elseif rawName ~= "" then
                local primary = rawName:match("^([^%s/]+)") or rawName
                local aliases = {}
                for alias in rawName:gmatch("/%s*([^%s/%[]+)") do
                    aliases[#aliases + 1] = alias
                end
                local meta = {
                    Name = cleanINC(primary),
                    RawName = cleanINC(rawName),
                    Description = cleanINC(rawDesc),
                    Section = sectionNames[sectionIndex] or ("Section " .. tostring(sectionIndex + 1)),
                    Aliases = aliases,
                    Scope = "Client-Sided",
                }
                ...
                commandOrder[#commandOrder + 1] = meta
            end
        end
    end

    -- Known local-only families. Explicit (CLIENT) markers always win.
    local clientSections = {
        ["General"] = true, ["Chat & Logs"] = true, ["Visuals"] = true,
        ["Camera"] = true, ["Lighting"] = true, ["Input & Mouse"] = true,
        ["Aliases"] = true, ["Plugins"] = true,
    }
    local clientWords = {
        console=true, oldconsole=true, explorer=true, dex=true, moondex=true, mdex=true,
        remotespy=true, rspy=true, simplespy=true, sspy=true, audiologger=true, alogger=true,
        jobid=true, notifyjobid=true, guiscale=true, volume=true, antilag=true, boostfps=true,
        lowgraphics=true, record=true, screenshot=true, togglefullscreen=true, togglefs=true,
        notify=true, notifyping=true, ping=true, norender=true, render=true,
        use2022materials=true, unuse2022materials=true, alignmentkeys=true,
        unalignmentkeys=true, ctrllock=true, unctrllock=true, firstp=true, thirdp=true,
        freecam=true, unfreecam=true, freecamspeed=true, fcspeed=true, fov=true,
        maxzoom=true, minzoom=true, camdistance=true, fixcam=true, restorecam=true,
        enableshiftlock=true, enablesl=true, lookat=true, xray=true, unxray=true,
        noxray=true, loopxray=true, unloopxray=true, togglexray=true,
        esp=true, espteam=true, noesp=true, unesp=true, unespteam=true,
        esptransparency=true, chams=true, nochams=true, unchams=true, locate=true,
        unlocate=true, nolocate=true, fullbright=true, fb=true, loopfullbright=true,
        loopfb=true, unloopfullbright=true, unloopfb=true, ambient=true, day=true,
        night=true, nofog=true, brightness=true, globalshadows=true, gshadows=true,
        noglobalshadows=true, nogshadows=true, restorelighting=true, light=true,
        nolight=true, unlight=true, darkchat=true, bubblechat=true, unbubblechat=true,
        nobubblechat=true, chatwindow=true, unchatwindow=true, nochatwindow=true,
        mouseSensitivity=true,
    }
    local serverWords = {
        kill=true, hkill=true, handlekill=true, fling=true, unfling=true, flyfling=true,
        unflyfling=true, walkfling=true, unwalkfling=true, invisfling=true, antifling=true,
        unantifling=true, loopoof=true, unloopoof=true, mutevcs=true, unmuteallvoices=true,
        spawnpoint=true, spawn=true, nospawnpoint=true, nospawn=true, teleporttool=true,
        tptool=true, bring=true, clientbring=true, cbring=true, goto=true, tgoto=true,
        vgoto=true, loopgoto=true, unloopgoto=true, pulsetp=true, tpwalk=true,
        teleportwalk=true, tpposition=true, tppos=true, gotopart=true, gotomodel=true,
    }
    ...
    local function computeScope(meta)
        ...
        if clientWords[n] or clientSections[meta.Section] then
            return "Client-Sided"
        end
        if serverWords[n] then
            return "Server-Sided"
        end
        -- Player movement/character changes are normally client-originated; commands that
        -- explicitly act on other players or replicated world state are listed separately.
        if meta.Section == "Player Physics" or meta.Section == "Character" or meta.Section == "Animations" then
            return "Client-Sided"
        end
        return "Server-Sided"
    end

    for _, meta in ipairs(commandOrder) do
        meta.Scope = computeScope(meta)
    end

    local gui = Instance.new("ScreenGui")
    gui.Name = "IncognolsConvenientMenu"
    ...
    local function label(text, pos, size, color)
    ...
    local currentPage
    local showHome, showHelp, showCommands, showCLI

    showHome = function()
        clearContent()
        currentPage = "Home"
        local help = button("HELP", UDim2.fromOffset(200,48), UDim2.new(.5,-205,.5,-24))
        local cli = button("COMMAND LINE INTERFACE", UDim2.fromOffset(200,48), UDim2.new(.5,5,.5,-24))
        help.MouseButton1Click:Connect(showHelp)
        cli.MouseButton1Click:Connect(showCLI)
    end

    showHelp = function()
        clearContent()
        currentPage = "Help"
        label("HELP",UDim2.fromOffset(5,2),UDim2.new(1,-10,0,25))
        label("Select a command section.",UDim2.fromOffset(5,30),UDim2.new(1,-10,0,20),CONFIG.SubText)
        local sections = {"All Commands","Player","Client-Sided","Server-Sided"}
        local y = 60
        for _, name in ipairs(sections) do
            local b = button(name,UDim2.new(1,-10,0,38),UDim2.fromOffset(5,y))
            b.MouseButton1Click:Connect(function() showCommands(name) end)
            y += 43
        end
        local back = button("< Back",UDim2.fromOffset(70,30),UDim2.fromOffset(5,y+8))
        back.MouseButton1Click:Connect(showHome)
    end

    local function metaMatches(meta, category)
        ...
        if category == "Player" then
            ...
            return s=="Player Physics" or s=="Waypoints" or s=="Teleportation" or s=="Camera"
                or s=="Player Information & Fun" or s=="Character" or s=="Animations"
                or s=="Input & Mouse" or s=="Tools"
        end
        return false
    end

    local function parseVisibleCommand(commandText)
        local primary = commandText:match("^([^%s/]+)")
        return (primary or commandText):lower()
    end

    showCommands = function(category)
        clearContent()
        currentPage = category
        local back = button("< Back",UDim2.fromOffset(70,28),UDim2.fromOffset(5,2))
        back.MouseButton1Click:Connect(showHelp)
        label(category,UDim2.fromOffset(85,2),UDim2.new(1,-90,0,28))
        ...
        local function refresh()
            ...
            for _, meta in ipairs(commandOrder) do
                local queryMatch = q=="" or meta.RawName:lower():find(q,1,true) ~= nil or meta.Description:lower():find(q,1,true) ~= nil
                if queryMatch and metaMatches(meta,category) then
                    local entry = Instance.new("TextButton")
                    entry.Size = UDim2.new(1,-5,0,54)
                    ...
                    n.Size = UDim2.new(1,-145,0,20)
                    n.Position = UDim2.fromOffset(7,3)
                    n.BackgroundTransparency = 1
                    n.Text = PREFIX .. meta.RawName
                    n.TextColor3 = CONFIG.Accent
                    n.TextSize = 13
                    ...
                    scope.Size = UDim2.fromOffset(130,18)
                    scope.Position = UDim2.new(1,-137,0,4)
                    scope.BackgroundTransparency = 1
                    scope.Text = meta.Scope == "Client-Sided" and "CLIENT" or "SERVER"
                    scope.TextColor3 = meta.Scope == "Client-Sided" and CONFIG.Client or CONFIG.Server
                    scope.TextSize = 10
                    scope.Font = Enum.Font.Code
                    scope.TextXAlignment = Enum.TextXAlignment.Right
                    scope.Parent = entry

                    local d = Instance.new("TextLabel")
                    d.Size = UDim2.new(1,-14,0,21)
                    d.Position = UDim2.fromOffset(7,26)
                    d.BackgroundTransparency = 1
                    d.Text = meta.Description
                    d.TextColor3 = CONFIG.SubText
                    d.TextSize = 11
                    ...
                    entry.MouseButton1Click:Connect(function()
                        ...
                        task.defer(function()
                            if _G.IncognoLCLIInput then
                                _G.IncognoLCLIInput.Text = PREFIX .. parseVisibleCommand(meta.RawName) .. " "
                                _G.IncognoLCLIInput:CaptureFocus()
                                _G.IncognoLCLIInput.CursorPosition = #_G.IncognoLCLIInput.Text + 1
                            end
                        end)
                    end)
                end
            end
            task.defer(function() list.CanvasSize = UDim2.fromOffset(0,layout.AbsoluteContentSize.Y+6) end)
        end
        search:GetPropertyChangedSignal("Text"):Connect(refresh)
        refresh()
    end

    local function printLine(console, layout, text)
        ...
        line.Parent = console
        task.defer(function()
            console.CanvasSize = UDim2.fromOffset(0,layout.AbsoluteContentSize.Y+8)
            console.CanvasPosition = Vector2.new(0,math.max(0,console.AbsoluteCanvasSize.Y))
        end)
    end

    local function displayHelp(console, layout, category)
        printLine(console,layout,"==== " .. category:upper() .. " ====")
        for _, meta in ipairs(commandOrder) do
            if metaMatches(meta,category) then
                printLine(console,layout,PREFIX .. meta.RawName .. " - " .. meta.Description)
            end
        end
        printLine(console,layout,"==== END ==== ")
    end
    ...
    showCLI = function()
        ...
        local back = button("< Back",UDim2.fromOffset(70,28),UDim2.fromOffset(5,2))
        back.MouseButton1Click:Connect(showHome)
        label("Command Line Interface",UDim2.fromOffset(85,2),UDim2.new(1,-90,0,28))
        ...
        input.Text = ""
        input.PlaceholderText = ";command target args..."
        ...
        local execute = button("EXECUTE",UDim2.fromOffset(78,38),UDim2.new(1,-165,1,-43))
        local clear = button("CLEAR",UDim2.fromOffset(78,38),UDim2.new(1,-82,1,-43))

        local function clearConsole()
            for _, child in ipairs(console:GetChildren()) do if child ~= clayout then child:Destroy() end end
            console.CanvasPosition = Vector2.zero
        end

        local function sendCommand(text)
            text = tostring(text or ""):match("^%s*(.-)%s*$") or ""
            if text == "" then return end
            printLine(console,clayout,"> " .. text)

            local clean = text
            if clean:sub(1,1)==";" or clean:sub(1,1)=="/" then clean=clean:sub(2) end
            local words = {}
            for token in clean:gmatch("%S+") do words[#words+1]=token end
            local commandName = (words[1] or ""):lower()
            table.remove(words,1)
            if commandName=="clear" then clearConsole(); return end
            if commandName=="help" then displayHelp(console,clayout,"All Commands"); return end
            ...
            local cmdObj = findCmd and findCmd(resolved) or nil
            if not cmdObj then
                printLine(console,clayout,"Unknown command: " .. resolved)
                return
            end

            -- execCmd is the actual original command engine. This preserves its
            -- target selectors, loops, repeat prefixes, aliases and command functions.
            pcall(function()
                execCmd(table.concat({resolved, table.unpack(words)}, " "), LocalPlayer, true)
            end)
        ...
        clear.MouseButton1Click:Connect(clearConsole)
        ...
        printLine(console,clayout,"IncognoL's Convenient Menu")
        printLine(console,clayout,"Command Line Interface ready.")
        printLine(console,clayout,"Type /help to show the complete command catalogue.")
        printLine(console,clayout,"Commands execute through the original INC command engine.")
    end

    -- GUI scale command is redirected to the new GUI.
    pcall(function()
        overridecmd("guiscale", function(args)
            local value = tonumber(args[1]) or 1
            local uiScale = main:FindFirstChild("INCScale")
            if not uiScale then
                uiScale = Instance.new("UIScale")
                uiScale.Name = "INCScale"
                uiScale.Parent = main
            end
            uiScale.Scale = math.clamp(value,0.4,2)
            notify("INC", "GUI scale: " .. tostring(uiScale.Scale))
        end)
    end)
    ...
    toggle.MouseButton1Click:Connect(function()
        if hidden then hidden=false; main.Visible=true else main.Visible=not main.Visible end
    end)
